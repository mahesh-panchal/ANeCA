...
public class Spring implements GraphAlgorithm
  {
		 ...
		 for (v = G.firstNode(); v != null; v = G.nextNode(v))
			for (u = G.nextNode(v); u != null; u = G.nextNode(u))
			   if ((v.getPosition().x == u.getPosition().x)
				 ...
				 {
				  vi = enumerate(G.getIndexFromNode(v));
				  ui = enumerate(G.getIndexFromNode(u));
	  ...
	  while((Boolean.TRUE).booleanValue())
		{
		 ...
		 for (v = G.firstNode(); v !=null; v = G.nextNode(v))
		   {
		   delta[enumerate(G.getIndexFromNode(v))]=Math.sqrt(findDelta2(G,v));
		   if (delta[enumerate(G.getIndexFromNode(v))] > maxDel)
			  maxDel = delta[enumerate(G.getIndexFromNode(v))];
		   if (NUM_TIMES_MOVED[enumerate(G.getIndexFromNode(v))] > maxTimes)
			  maxTimes = NUM_TIMES_MOVED[enumerate(G.getIndexFromNode(v))];
		   }

		 mv = G.firstNode(); mvi = enumerate(G.getIndexFromNode(mv));
		 maxValue = 0;
		 for (v = mv; v !=null; v = G.nextNode(v))
		   {
			if (maxTimes != 0)
			   tempValue = TempFunction 
	         (delta[enumerate(G.getIndexFromNode(v))]/maxDel,
	         1 - NUM_TIMES_MOVED[enumerate(G.getIndexFromNode(v))]/maxTimes);
			else
			   // the first time, base the decision only upon the delta
			   tempValue = delta[enumerate(G.getIndexFromNode(v))]/maxDel;
			if (tempValue > maxValue)
			  {
			   mv = v; mvi = enumerate(G.getIndexFromNode(mv));
			   maxValue = tempValue;
			  }
		   }
   ...
   private static int enumerate(int Index)
	 {
	  int i;
	  ...
	  for (s = G.firstNode(); s !=null; s = G.nextNode(s))
		{
		 ...
		 si = enumerate(G.getIndexFromNode(s));
		 done[si] = true;
		 for (v = G.firstNode(); v !=null; v = G.nextNode(v))
		 ...
		 while (!(queue.isEmpty()))
		   {
			...
			pi = enumerate(G.getIndexFromNode(p));
			vi = enumerate(G.getIndexFromNode(v));
			D[si][vi] = D[pi][si] + 1;
			D[vi][si] = D[si][vi];
   ...
   private static void MoveToNewPosition(Graph G)
	 {
	  ...
	  int mvi = enumerate(G.getIndexFromNode(mv));
	  findPartials(G,mv);
			A = partial_xx;
			B = partial_xy;
			C = -(partial_x);
