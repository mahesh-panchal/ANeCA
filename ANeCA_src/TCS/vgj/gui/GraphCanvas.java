...
import TCS.VGJ.util.DRect;
import TCS.VGJ.util.Matrix44;
import TCS.VGJ.graph.Graph;
import TCS.VGJ.graph.Node;
import TCS.VGJ.graph.Edge;
import TCS.VGJ.graph.NodePropertiesDialog;
import TCS.VGJ.graph.EdgePropertiesDialog;
...
public class GraphCanvas extends OffsetCanvas
		 implements GraphUpdate, KeyListener, MouseListener, MouseMotionListener
{
...
///////////////////////////////////////////////////////////////////////////////
	...
	public GraphCanvas(Graph graph_in, JFrame frame_in)
	{
		addKeyListener(this);
	...
	public void center()
	{
		...
		setOffsets_(.5 * ((double)windowSize_.width - (minx_ + maxx_) * scale_), .5 * ((double)windowSize_.height - (miny_ + maxy_) * scale_));
		getParent().dispatchEvent(new TCSEvent((Object) this, RESIZE, (Object) this));
	...
	private synchronized void drawAxes_(Graphics graphics)
	{
		...
		p2.transform(rotxMatrix_);
		letter = new String("X");
		lx = p2.x - (double)fm.stringWidth(letter) / 2.0;
		ly = p2.y + (double)fm.getAscent() / 2.0;
	...
	{
		DPoint3 retval  = new DPoint3((double)windowSize_.width / 2.0,(double) windowSize_.height / 2.0, 0);
	...
	private String getLabel_(double x, double y, double z, boolean mousein)
	{
		...
		if(string.equals("")){ // fix for jumping scrollbars
			string = ScrolledPanel.DEFAULT_LABEL;
		}
	...
	public void groupControl(char key)
	{
	...
	public void mouseExited(MouseEvent e){
		...
		getParent().dispatchEvent(new TCSEvent((Object) this, OffsetCanvas.LABEL, string));
	}
	public void mousePressed(MouseEvent e){
		...
		//if(e.isMetaDown() && e.isAltDown() && e.isControlDown()) // wrong! The statement is checking these are not set!
		if((e.getModifiersEx() & (InputEvent.BUTTON1_DOWN_MASK | InputEvent.ALT_DOWN_MASK| InputEvent.CTRL_DOWN_MASK )) == InputEvent.BUTTON1_DOWN_MASK) // looking to see if button 1 was pressed without the Alt or Control keys
		{
		// button 1
		...
		//else if ((e.modifiers & (Event.ALT_MASK | Event.CTRL_MASK)) != 0) // can't figure out what this is asking
		//else if (e.isAltDown() || e.isControlDown()) // is this correct? no!
		else if ((e.getModifiersEx() & (InputEvent.BUTTON2_DOWN_MASK | InputEvent.ALT_DOWN_MASK| InputEvent.CTRL_DOWN_MASK )) == InputEvent.BUTTON2_DOWN_MASK)
		{
			// button 2
			currentMouseAction_ = 2;
		}
		...
		{
			// Select object.
			selected_ = NONE_;			if (selectedEdge_ != null && e.getClickCount() == 2)
			{
			...
			if (selected_ == NONE_)
			{
				//if ((e.modifiers & Event.SHIFT_MASK) == 0)
				if(!e.isShiftDown())
				//if((e.getModifiersEx() & InputEvent.SHIFT_DOWN_MASK) == 0) // the shift key is not down
				{
					unselectItems();
				}
	...
	/*public void processMouseEvent(MouseEvent e){
		if(e.getID() == MouseEvent.MOUSE_PRESSED){
			...
			// Determine which action to take.
			//if ((e.modifiers & (Event.META_MASK | Event.ALT_MASK | Event.CTRL_MASK)) == 0)
			if(e.isMetaDown() && e.isAltDown() && e.isControlDown()) // check
			{
			// button 1
			...
			else if (e.isAltDown() || e.isControlDown()) // is this correct? MP
			{
				// button 2
				currentMouseAction_ = 2;
			}
			...
			else if (currentMouseAction_ == 3)
			{
				...
				if (selectedEdge_ != null && e.getClickCount() == 2)
				{
					setEdgeProperties(false);
					return;
				}
	...
	public void setDirected(boolean directed)
	{
		...
		getParent().dispatchEvent(new TCSEvent((Object) this, UPDATE, null));
	}
	...
	public String toPS(double width, double height, double pagewidth, double pageheight, double fontsize, double margin, double overlap, boolean landscape)
	{
		...
		// Get graph drawn boundaries.		DPoint graphwidth   = new DPoint();
		...
		int page            = 1;		for (int w = 0; w < pages_wide; w++)
		{
...
//		System.out.println("this is page_height " + pages_high);
		...
		//result += tmpnode.toPSimage(i, viewTransform_);
		double minx         = 10000;
		//result += tmpnode.toPSimage(i, viewTransform_);		double tempx;
		//result += tmpnode.toPSimage(i, viewTransform_);
		double minwid       = 0;
		//result += tmpnode.toPSimage(i, viewTransform_);
		double maxx         = -10000;
