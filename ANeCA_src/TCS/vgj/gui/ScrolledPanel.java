 ...
 * @see        OffsetCanvas
 */
public class ScrolledPanel extends LPanel implements AdjustmentListener
{
	...
	final static String DEFAULT_LABEL = " ";

	public static Scrollbar vscroll_, hscroll_;
	...
	public void center()
	{
		...
		getParent().dispatchEvent(new TCSEvent(this, OFFSET, this));
	}
	...
	private void configure()
	{
		...
		Dimension tmpD  = offsetCanvas_.getSize();
	...
	public void processEvent(AWTEvent e)
	{
		if(e instanceof TCSEvent){
			...
			{
				AWTEvent event  = (AWTEvent) ((TCSEvent) e).getInformation();
				if (event.getSource() instanceof Scrollbar &&
						(event.getID() == AdjustmentEvent.ADJUSTMENT_VALUE_CHANGED ||
						event.getID() == AdjustmentEvent.UNIT_DECREMENT ||
						event.getID() == AdjustmentEvent.UNIT_INCREMENT ||
						event.getID() == AdjustmentEvent.BLOCK_DECREMENT ||
						event.getID() == AdjustmentEvent.BLOCK_INCREMENT))
				{
					// Scrollbar event
					...
					// post an event indicating a scroll
					getParent().dispatchEvent(new TCSEvent((Object) this, OFFSET, (Object) this));

					return;
				}
				else if (event.getSource() instanceof OffsetCanvas)
				{
					if (event.getID() == OffsetCanvas.RESIZE)
					{
					...
					else if (event.getID() == OffsetCanvas.LABEL)
					{
						String string  = (String) ((TCSEvent) event).getInformation();
