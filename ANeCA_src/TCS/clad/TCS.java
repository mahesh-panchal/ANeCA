	...
	public boolean runTCS(String infile, String outfile, boolean gapmode, boolean distances, String logFileName) {
		...
		if (components.size() == 1) {
			if(!Mediator.isCommandLineApp()){
				...
				Mediator.writeError("There is only one haplotype.\nA graph cannot be built.[TCS]\n]"); // causes exit!
				if (warnMissing) {
					//JOptionPane.showMessageDialog(frame, "Warning : potential missing data ambiguities" + "\nwhen collapsing sequences to haplotypes" + "\nCheck the logfile.\n", "TCS warning", JOptionPane.ERROR_MESSAGE);
					Mediator.writeError("Warning : potential missing data ambiguities\nwhen collapsing sequences to haplotypes\nCheck the logfile.\n");
				}
			}
		...
		while (enumRealTaxa.hasMoreElements()) {
			...
			//	TCSIO.printGraph("graph" + graphNumber + ".out", components, distances, Log, maxWtaxa, maxNameLen);
			//}
		...
		if (graphExists) {
			...
			if(!Mediator.isCommandLineApp()){
				...
				Mediator.writeError("No connection below limit.\nA graph cannot be built.\n[TCS]");
			}
			return false;
		}
		...
		Log.dprintln("\nIt took " + (double)(endTime - startTime) / 60000.0 + " minutes.");
		...
		if (warnMissing) {
			if(!Mediator.isCommandLineApp()){
				...
				str.append("Potential missing data ambiguities");
				str.append("\nwhen collapsing sequences to haplotypes.");
				str.append("\nCheck the logfile.\n");
				str.append("[TCS]\n");
				Mediator.writeWarning(str.toString());
			}
		}
		return graphExists;
	}
		...
		for (nodegml = gml.getGMLSubObject("node", GMLobject.GMLlist, false); nodegml != null; nodegml = gml.getNextGMLSubObject()) {
			...
			if ((data = nodegml.getGMLSubObject("data", GMLobject.GMLlist, false)) != null) {
				...
				if ((tmp = (String)data.getValue("Frequency", GMLobject.GMLstring)) != null) {
					...
					String fr = tmp.trim().split("\\s+")[0];
					freq = Integer.parseInt(fr.split("=")[1]);
				}
			} else {
				intermediate = true;
				freq = 1;
...
//						}
//
//						if (char1 == '?' || char2 == '?' || char1 == 'N' || char2 == 'N') {
//							continue;
//						}
//						if (char1 != char2) {
//							realdist.distance++;
...
//		}
	...
	private int buildParseTable(int MaxNucleotides, int conLimit) {
		...
		Log.dprintln("_________________________________________________________");
	...
	private void calculateConnectionLimit() {
		...
		if (frame != null) {
			//
			if (frame.manualBox.isSelected()) {
				steps = frame.manualConnections.getText();
			} else {
				// conLimit = frame.conLimit.getSelectedIndex();
				String conL = (String) frame.conLimit.getSelectedItem();
		...
		if (steps.equals("")) {
			...
			try {
				...
				if(!Mediator.isCommandLineApp()){
					...
					Mediator.writeWarning("You must use a positive integer for the Connection Limit!\n\tUsing a default value of 40 for the Connection Limit[TCS]\n");
				}
