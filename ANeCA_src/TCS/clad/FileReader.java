...
public class FileReader {
	...
	private boolean printseq = false;
	private Logger Log;
	...
	*/
		...
		while (in.EOF() == false) {
			...
			for (j = 0; j < len; j++) {
				...
				if (c == '\n' || c == '\r') {
					in.close();
					if(!Mediator.isCommandLineApp()){
						JOptionPane.showMessageDialog(NCPA.getFrame(), "You have extra returns in sequence \"" + name + "\".",
						...
						str.append("You have extra returns in sequence \"");
						str.append(name);
						str.append("\".");
						str.append("[TCS]\n");
				...
				if (c == ' ') {
					in.close();
					if(!Mediator.isCommandLineApp()){
						JOptionPane.showMessageDialog(NCPA.getFrame(), "The length of sequence \"" + name + "\"" + " is not " + len + ".",
								"TCS warning", JOptionPane.WARNING_MESSAGE);
						//frame.dispose();
						//System.exit(0);
						Mediator.newTCSAnalysis();
						...
						StringBuffer str = new StringBuffer();
						str.append("The length of sequence \"");
						str.append(name);
						str.append("\" is not ");
						str.append(len);
						str.append(".");
				...
				if (c != '1' && c != '2' && c != '?') {
					in.close();
					if(!Mediator.isCommandLineApp()){
						JOptionPane.showMessageDialog(NCPA.getFrame(), "One of the characters in sequence \"" + name + "\" is not (12?).",
								"TCS warning", JOptionPane.WARNING_MESSAGE);
						//frame.dispose();
						//System.exit(0);
						Mediator.newTCSAnalysis();
						...
						StringBuffer str = new StringBuffer();
						str.append("One of the characters in sequence \"");
						str.append(name);
						str.append("\" is not (12?).");
						str.append("[TCS]\n");
						Mediator.writeError(str.toString());
					}
				}
				...
				if (in.EOF()) {
					in.close();
					if(!Mediator.isCommandLineApp()){
						JOptionPane.showMessageDialog(NCPA.getFrame(), "Unexpected end of file was encountered.",
								"TCS warning", JOptionPane.WARNING_MESSAGE);
						//frame.dispose();
						//System.exit(0);
						Mediator.newTCSAnalysis();
						...
						StringBuffer str = new StringBuffer();
						str.append("Unexpected end of file was encountered");
						str.append("[TCS]\n");
						Mediator.writeError(str.toString());
					}
				}
	...
	private void ReadMatrix(char c) {
		...
		for (popA = 0; popA < num; popA++) {
			...
			if (names[popA] == null) {
				in.close();
				if(!Mediator.isCommandLineApp()){
					JOptionPane.showMessageDialog(NCPA.getFrame(), "There was an error reading the sequence names" +
					...
					StringBuffer str = new StringBuffer();
					str.append("There was an error reading the sequence names");
					str.append("\nin the pairwise distance matrix");
					str.append("[TCS]\n");
					Mediator.writeError(str.toString());
				}
			...
			for (popB = 0; popB < popA; popB++) {
				...
				if (distance[popA][popB] < 1) {
					in.close();
					if(!Mediator.isCommandLineApp()){
						JOptionPane.showMessageDialog(NCPA.getFrame(), "There was an error reading the distance matrix." +
						...
						StringBuffer str = new StringBuffer();
						str.append("There was an error reading the distance matrix.");
						str.append("\nAre you using absolute distances?");
						str.append("\nAre you inputing HAPLOTYPE distances (no zeroes in the matrix)? : ");
						str.append(distance[popA][popB]);
						str.append("[TCS]\n");
						Mediator.writeError(str.toString());
					} 
				}
		...
		for (i = 0; i < num; i++) {
			...
			for (j = 0; j < len; j++) {
				...
				if (c == 'R' || c == 'M' || c == 'W' || c == 'S' || c == 'K' || c == 'Y' ||
					...
					if(IUPACcounter == 0){
					if(!Mediator.isCommandLineApp()){
						JOptionPane.showMessageDialog(NCPA.getFrame(), "Note that there are IUPAC ambiguity codes in the data: \"" + c + "\"" +
								"\n\nIUPAC codes: " +
						...
						StringBuffer str = new StringBuffer();
						str.append("Note that there are IUPAC ambguity codes in the data: \"");
						str.append(c);
						str.append("\"");
						str.append("\n\nIUPAC codes: ");
						str.append("\nM: A/C    S: G/C    H: A/C/T    B: C/G/T");
				...
				if (c != 'A' && c != 'C' && c != 'G' && c != 'T' && c != '-' && c != '.' &&
					...
					in.close();
					if(!Mediator.isCommandLineApp()){
						JOptionPane.showMessageDialog(NCPA.getFrame(), "One of the characters in sequence \"" + name + "\" is not (ACGCacgt.?N-)." +
								"\nnor a IUPAC ambiguity code (RMWSKYHBVDXN)",
								"TCS warning", JOptionPane.WARNING_MESSAGE);
						//System.exit(0);
						Mediator.newTCSAnalysis();
					} else {
	...
	private void ReadNexusDis() {
		...
		if (Character.isDigit((char)num)) {
			in.close();
			if(!Mediator.isCommandLineApp()){
				JOptionPane.showMessageDialog(NCPA.getFrame(),"ReadNexusDis > Problems reading the number of the sequences! : " + num,
				 "TCS warning", JOptionPane.WARNING_MESSAGE);		
				//System.exit(0);
				Mediator.newTCSAnalysis();
			} else {
				StringBuffer str = new StringBuffer();
				str.append("ReadNexusDis > Prolems reading the number of the sequences! : ");
				str.append(num);
				str.append("[TCS]\n");
				Mediator.writeError(str.toString());
			}
		...
		if (Character.isDigit((char)len)) {
			in.close();
			if(!Mediator.isCommandLineApp()){
				JOptionPane.showMessageDialog(NCPA.getFrame(),"ReadNexusDis > Problems reading the length of the sequences! : " + len,
			 		"TCS warning", JOptionPane.WARNING_MESSAGE);		
				//System.exit(0);
				Mediator.newTCSAnalysis();
			} else {
				StringBuffer str = new StringBuffer();
				str.append("ReadNexusDis > Problems reading the length of the sequences! : ");
				str.append(len);
				str.append("[TCS]\n");
				Mediator.writeError(str.toString());
			}
		...
		for (int i = 0; i < num; i++) {
			...
			for (j = 0; j < len; j++) {
				...
				if (c == 'R' || c == 'M' || c == 'W' || c == 'S' || c == 'K' || c == 'Y' ||
					...
					if(IUPACcounter == 0){
			  		if(!Mediator.isCommandLineApp()){
						JOptionPane.showMessageDialog(NCPA.getFrame(),"There are IUPAC ambiguity codes in the data: \"" + c + "\"" +
						"\n\nIUPAC codes: " + 
						"\nM: A/C    S: G/C    H: A/C/T    B: C/G/T" +
		  				"\nR: A/G    K: G/T    V: A/C/G    X: A/C/G/T = ?" +
		  				"\nW: A/T    Y: C/T    D: A/G/T    N: A/C/G/T = ?",
						...
						StringBuffer str = new StringBuffer();
						str.append("There are IUPAC ambiguity codes in the data: \"");
						str.append(c);
						str.append("\"\n\nIUPAC codes: ");
						str.append("\nM: A/C    S: G/C    H: A/C/T    B: C/G/T");
						str.append("\nR: A/G    K: G/T    V: A/C/G    X: A/C/G/T = ?");
				...
				if (c != 'A' && c != 'C' && c != 'G' && c != 'T' && c != '-' && c != '.' &&
					...
					in.close();
			 		if(!Mediator.isCommandLineApp()){
						JOptionPane.showMessageDialog(NCPA.getFrame(),"One of the characters in sequence \"" + name + "\" is not (ACGTacgt.?N-)." +
						"\nnor a IUPAC ambiguity code (RMWSKYHBVDXN)" ,
					 	"TCS warning", JOptionPane.WARNING_MESSAGE);
						//System.exit(0);
						Mediator.newTCSAnalysis();
	...
	private void ReadPhylipDis(String line) {
		...
		if (Character.isDigit((char)num)) {
			in.close();
			if(!Mediator.isCommandLineApp()){
				JOptionPane.showMessageDialog(NCPA.getFrame(),"ReadPhylipDis > Problems reading the number of the sequences! : " 
					+ num, "TCS warning", JOptionPane.WARNING_MESSAGE);		
				//System.exit(0);
				Mediator.newTCSAnalysis();
			} else {
				StringBuffer str = new StringBuffer();
				str.append("ReadPhylipDis > Problems reading the number of the sequences! : ");
				str.append(num);
				str.append("[TCS]\n");
				Mediator.writeError(str.toString());
			}
		...
		if (Character.isDigit((char)len)) {
			in.close();
			if(!Mediator.isCommandLineApp()){
				JOptionPane.showMessageDialog(NCPA.getFrame(),"ReadPhylipDis > Problems reading the length of the sequences! : " 
					+ len, "TCS warning", JOptionPane.WARNING_MESSAGE);		
				//System.exit(0);
				Mediator.newTCSAnalysis();
			} else {
				StringBuffer str = new StringBuffer();
				str.append("ReadPhylipDis > Problems reading the length of the sequences! : ");
				str.append(len);
				str.append("[TCS]\n");
				Mediator.writeError(str.toString());
			}
